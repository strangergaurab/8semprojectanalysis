      ...
      messages: [
        {
          role: "system",
          content:
            "You are a helpful assistant that provides informative responses in markdown format. Use headings, lists, and code blocks where appropriate.",
        },
        {
          role: "user",
          ...
          **User's Answer:** ${userAnswer}  
          ...
          If the user's answer is correct, prioritize it. If incomplete or wrong, improve or correct it concisely. Return in markdown format.`,
        },
      ],
    });

    return NextResponse.json(
      { success: true, data: chat.choices[0].message.content },
      { status: 200 }
    );
  } catch (error) {
    return handleError(error, "api") as APIErrorResponse;
