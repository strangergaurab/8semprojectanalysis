...
// Enhanced DataGrid Component with Revenue API Integration
export const DataGrid = ({ isDashboard = false }) => {
  const [totalUsers, setTotalUsers] = useState<number | null>(null);
  const [totalRevenue, setTotalRevenue] = useState<number | null>(null);
  const [totalPremiumUsers, setTotalPremiumUsers] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const [usersResponse, premiumuserResponse, revenueResponse] =
          await Promise.all([
            fetch(`${process.env.NEXT_PUBLIC_API_URL}api/admin/count/users/`, {
              headers: {
                Authorization: `Bearer ${authTokens?.access_token}`,
              },
            }),
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}api/admin/count/premium-users/`,
              {
                headers: {
                  Authorization: `Bearer ${authTokens?.access_token}`,
                },
            ...
            fetch(
              `${process.env.NEXT_PUBLIC_API_URL}api/admin/count/revenue/`,
              {
                headers: {
                  Authorization: `Bearer ${authTokens?.access_token}`,
                },
        ...
        if (
          !usersResponse.ok ||
          !premiumuserResponse.ok ||
          !revenueResponse.ok
        ) {
          throw new Error("Failed to fetch data");
        }

        const [usersData, premiumuserData, revenueData] = await Promise.all([
          usersResponse.json(),
          premiumuserResponse.json(),
          revenueResponse.json(),
        ]);

        setTotalUsers(usersData.no_of_users);
        setTotalRevenue(revenueData.total_revenue);
        setTotalPremiumUsers(premiumuserData.premium_users);
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      <DataCard
        title="Premium Users"
        value={totalPremiumUsers !== null ? totalPremiumUsers : 0}
        icon={UsersRound}
        dateRange={dateRangeLabel}
      ...
      <DataCard
        title="Revenue"
        value={totalRevenue !== null ? totalRevenue : 0}
        icon={DollarSign}
        dateRange={dateRangeLabel}
